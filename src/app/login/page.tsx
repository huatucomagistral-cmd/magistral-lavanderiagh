      ...
      <div className="w-full max-w-5xl grid lg:grid-cols-2 gap-6 relative z-10">
        ...
        <div className="glass-card p-8 sm:p-10 flex flex-col justify-center">
          ...
          <div className="space-y-6">
            <div>
              <p className="text-sm font-medium text-foreground/70 mb-3">Administradores</p>
            ...
            <div className="flex items-center gap-4 text-foreground/30 text-xs uppercase font-bold tracking-widest my-2">
            ...
            <form onSubmit={handleEmailLogin} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-foreground/70 mb-1">Correo</label>
                <div className="relative">
                  <Mail className="absolute left-3 top-1/2 -translate-y-1/2 text-foreground/30" size={18} />
                  <input type="email" value={email} onChange={e => setEmail(e.target.value)} required
                    className="w-full bg-white border border-black/10 rounded-xl pl-10 pr-4 py-3 text-foreground focus:outline-none focus:ring-2 focus:ring-primary transition-all placeholder:text-foreground/30"
              ...
              <button type="submit" disabled={isLoading} className="w-full bg-primary hover:bg-primary-hover active:scale-95 transition-all text-white font-bold rounded-xl py-3.5 flex items-center justify-center disabled:opacity-50 gap-2 shadow-sm">
        ...
        <div className="rounded-2xl p-8 sm:p-10 flex flex-col justify-center items-center text-center bg-gradient-to-br from-primary/20 to-accent/10 border border-white/5 relative overflow-hidden">
          ...
          <h2 className="text-3xl font-bold text-foreground mb-4">Abre tu Lavandería Digital</h2>

          <div className="w-full max-w-sm bg-white border border-black/5 p-6 rounded-2xl relative z-10 shadow-xl">
            <h3 className="text-lg font-semibold text-foreground mb-4">Comienza Completamente Gratis</h3>
            <button onClick={handleGoogleLogin} disabled={isLoading} className="w-full bg-white hover:bg-gray-50 active:scale-95 transition-all text-foreground font-bold rounded-xl py-3.5 flex items-center justify-center gap-3 disabled:opacity-50 border border-black/10 shadow-sm">
               <svg viewBox="0 0 24 24" width="20" height="20" xmlns="http://www.w3.org/2000/svg">
                  <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
                  <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
                  <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
