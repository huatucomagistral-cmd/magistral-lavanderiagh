...
import { UserPlus, ShieldAlert, Mail, Trash2, Smartphone, Users } from "lucide-react";
...
export default function StaffPage() {
  ...
  const shareViaWhatsApp = (personEmail: string) => {
    const text = `¡Hola! Te he dado acceso al sistema de Lavandería Magistral.\n\nEstas son tus credenciales de acceso:\nUsuario: ${personEmail}\nContraseña: [Escribe aquí la contraseña]\n\nIngresa aquí:\nhttps://lavanderia.magistral.pe/`;
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-white/60">Agrega empleados a la plataforma. Recibirán un correo para crear su contraseña.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1">
          <form onSubmit={handleInvite} className="glass-card p-6 sticky top-24 space-y-4">
             ...
             <button type="submit" disabled={isInviting} className="w-full bg-primary hover:bg-primary-hover active:scale-95 transition-all text-white font-bold rounded-xl px-4 py-3 flex items-center justify-center gap-2 mt-4">
               {isInviting ? <span className="animate-spin border-2 border-white/30 border-t-white rounded-full w-5 h-5 mx-auto" /> : "Crear Acceso"}
             </button>
             ...
             <div className="flex gap-2 items-start mt-4 p-3 rounded-lg bg-surface border border-white/5">
                ...
                <p className="text-xs text-white/50 leading-relaxed">
                  El rol <strong className="text-white">PERSONAL</strong> tiene permisos limitados: solo puede gestionar Caja y Pedidos. No verá reportes de dinero.
                </p>
             </div>
          </form>
        </div>
        ...
        <div className="lg:col-span-2 space-y-4">
           {staff.map(person => (
             <div key={person.id} className="glass-card p-4 sm:p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 group transition-colors hover:bg-white/[0.02]">
               <div className="flex items-center gap-4">
                 <div className="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center bg-gradient-to-br from-surface to-background text-lg font-bold text-white uppercase">
               ...
               <div className="w-full sm:w-auto flex flex-row items-center justify-between sm:justify-end gap-3 border-t border-white/5 pt-4 sm:border-0 sm:pt-0">
                  {person.role !== "ADMIN" && (
                      <button type="button" onClick={() => shareViaWhatsApp(person.email)} title="Avisar por WhatsApp" className="text-white/50 hover:text-[#25D366] transition-colors p-2 rounded-lg hover:bg-white/5">
                        <Smartphone size={20} />
                      </button>
                  )}
                  
                  {person.role !== "ADMIN" && (
                     <button onClick={() => handleRevoke(person.email)} className="text-xs font-semibold text-error/80 hover:text-white hover:bg-error px-3 py-1.5 rounded-lg border border-transparent hover:border-error/20 transition-all flex items-center gap-1.5">
